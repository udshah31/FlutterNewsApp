...
class _ListViewPageState extends State<ListViewPage> {
  ...
  void initState() {
    super.initState();
    NewsService.getNewsPaperNews().then((dataFromServer) {
      setState(() {
        data = dataFromServer;
      });
    });
  ...
  void _onRefresh() async {
    ...
    var list = await NewsService.getNewsPaperNews(page: 1);
    data.clear();
    page = 1;
    data.addAll(list);
    setState(() {
  ...
  void _onLoading() async {
    print("_onloading");
    page++;
    var list = await NewsService.getNewsPaperNews(page: page);
    data.addAll(list);
    await Future.delayed(Duration(seconds: 2));
    setState(() {
